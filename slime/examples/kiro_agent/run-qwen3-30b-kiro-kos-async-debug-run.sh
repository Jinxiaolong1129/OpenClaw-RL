#!/bin/bash
# =============================================================================
# SLIME Async Training with Remote Kiro-on-Strands (KoS) Rollout Server
# =============================================================================
#
# Async variant of run-qwen3-30b-kiro-kos.sh.
# Uses train_async.py to overlap rollout(N+1) with train(N), reducing trainer
# idle time. With --rollout-external (no --colocate), rollout engines run on
# separate GPUs so async overlap is safe.
#
# Key differences from the sync script:
#   - Calls train_async.py instead of train.py
#   - Adds --update-weights-interval to control weight sync frequency
#
# Prerequisites are identical to the sync script — see run-qwen3-30b-kiro-kos.sh.
# =============================================================================
...
SGLANG_RUN_NAME=${SGLANG_RUN_NAME:-"smoke_test_async"}
...
# KoS env file written by launch_kos_only.sh (disaggregated CPU nodes).
# Falls back to deriving URLs from SGLANG_ENGINE_ADDRS (co-located mode).
KOS_ENV_FILE=${KOS_ENV_FILE:-"${SGLANG_ENV_BASE}/${KOS_RUN_NAME}/kos_servers.env"}

if [ -z "${KOS_REMOTE_URLS:-}" ]; then
    # Try 1: Read from kos_servers.env (disaggregated deployment)
    ...
    # Try 2: Derive from SGLANG_ENGINE_ADDRS (co-located deployment, backward compat)
    if [ -z "${KOS_REMOTE_URLS:-}" ]; then
        _SEEN_IPS=""
        _KOS_URLS=""
...
export VALIDATE_SHARD_MAP_DIR=${VALIDATE_SHARD_MAP_DIR:-""}
export VALIDATE_SERVICE_PORT=${VALIDATE_SERVICE_PORT:-51429}
export VALIDATE_SERVICE_NAMESPACE=${VALIDATE_SERVICE_NAMESPACE:-""}
export KOS_BINARY_REWARD=${KOS_BINARY_REWARD:-"true"}
...
echo "=============================================="
echo "Async Training Cluster Configuration:"
...
CKPT_ARGS=(
   ...
   --load /mnt_out/myshang/models/qwen/kiro_rl/Qwen3-30B_slime_kos/${RUN_NAME}
   --save /mnt_out/myshang/models/qwen/kiro_rl/Qwen3-30B_slime_kos/${RUN_NAME}
   --save-interval 10
)
...
#    --prompt-data /mnt_out/zhenghuj/data/preprocessed_sweap_530_v8.0_train.parquet
   --prompt-data /mnt_out/kangruir/datasets/preprocessed_sweap_filtered_301_v8.1_train.parquet
   ...
   --rollout-max-response-len 73728
...
EVAL_ARGS=(
   ...
   --eval-max-response-len 73728
   --eval-top-p 1
)

# =============================================================================
# Performance / Parallelism (all configurable via env / YAML)
# =============================================================================
...
# Boolean flags — only add if enabled
...
TENSORBOARD_ARGS=(
   --use-tensorboard
   --tb-project-name slime-smoke-test-async
   --tb-experiment-name qwen3-30b-rl-async
)
...
MLFLOW_ARGS=(
   --use-mlflow
   --mlflow-experiment-name slime-kos-smoke-test-async
   --log-memory-to-mlflow
   --mlflow-run-name "qwen3-30b-kos-async-${RUN_NAME}"
)
...
# =============================================================================
# --update-weights-interval controls how often weights are synced to SGLang
# engines. In async mode, rollout(N+1) runs with weights from the last sync.
# Interval=1 syncs every step (most on-policy, but pauses rollout each time).
# Interval=2+ reduces sync overhead at the cost of slightly staler rollout data.
...
CUSTOM_ARGS=(
   ...
   # --custom-rm-path examples.kiro_agent.kiro_generate_with_kos.execution_reward_func
)
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"VALIDATE_SHARD_MAP_DIR\": \"${VALIDATE_SHARD_MAP_DIR}\",
    \"VALIDATE_SERVICE_PORT\": \"${VALIDATE_SERVICE_PORT}\",
    \"VALIDATE_SERVICE_NAMESPACE\": \"${VALIDATE_SERVICE_NAMESPACE}\",
    \"KOS_BINARY_REWARD\": \"${KOS_BINARY_REWARD}\",
...
LOG_FILE="${LOG_DIR}/train_async_kos_${NUM_NODES}nodes_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$(dirname "$LOG_FILE")"
echo "Training log: $LOG_FILE"

echo "=============================================="
echo "Starting ASYNC Training with External SGLang + Remote KoS Rollout:"
